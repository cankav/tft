function [dot_text node_offset] = write_dot_svg(adj_mat, cluster_id, node_offset, gtp_groups_id, base_filename, parent_cluster_id)
    ...
    cmd = ['cd dot_files && dot -Tsvg ' base_filename '_gtpmodel_wds_group_' num2str(gtp_groups_id) '_cluster' num2str(cluster_id) '.dot > ' base_filename '_gtpmodel_wds_group_' num2str(gtp_groups_id) '_cluster' num2str(cluster_id) '.svg' ];
    system(cmd);
